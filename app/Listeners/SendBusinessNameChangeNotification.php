...
use App\Models\Subscribers; // Use Subscribers model instead of User
...
class SendBusinessNameChangeNotification
{
    public function handle(BusinessNameUpdated $event)
    {
        ...
        $subscribers = Subscribers::whereNotNull('email_verified_at')->get();
        ...
        foreach ($subscribers as $subscriber) {
            ...
            try {
                Mail::html($emailBody, function ($message) use ($subscriber, $event) {
                    $message->to($subscriber->email)
                        ->subject("Business Information Change Notification for {$event->newName}");
                });
            } catch (\Exception $e) {
                \Log::error('Error sending email to ' . $subscriber->email . ': ' . $e->getMessage());
            }
        }
    }
}
