...
<template>
  ...
  >
    <!-- Title Section -->
    <div class="pt-28 flex flex-col items-center p-3 mb-20"> <!-- Added "mb-20" for spacing -->
      <p
    ...
    <!-- Product Packages -->
    <div
      class="flex flex-wrap justify-center gap-8 w-full max-w-screen px-4"
    >
      <div
        v-for="(packageItem, index) in packageData"
        :key="packageItem.id"
        class="flex flex-col w-[420px] h-auto rounded-xl shadow-2xl bg-gradient-to-br from-gray-100 via-gray-200 to-gray-100 border border-gray-300 transition-all duration-300 hover:scale-105 hover:shadow-3xl overflow-hidden group"
      >
        <!-- Header -->
        <div
        ...
        <!-- Footer -->
        <p
          class="text-gray-800 text-[20px] h-[60px] text-center bg-gradient-to-r from-gray-200 to-gray-300 rounded-b-xl flex items-center justify-center sticky bottom-0 z-10 transition-colors duration-300 group-hover:from-gray-300 group-hover:to-gray-400"
        >
          {{ textAreasPackage[`card${index + 1}Package`].value }}
        </p>
      </div>
    </div>
  </div>
